//configuration:
const LEARNS_FROM = ["the-chamber"] //channels the bot learns from but doesnt talk in
const TALKS_IN = ["the-chamber"] //channels the bot will talk to people in
...
bot.on("message", async message => {
	...
	if(!keys.includes(message.cleanContent) && message.content.length > 0 && LEARNS_FROM.includes(message.channel.name) && !message.content.includes("discord.gg")) {
		...
		await fs.writeFile("./db.json", JSON.stringify(db), err => {
			if(err) throw err;
		}) //ctrl+s
